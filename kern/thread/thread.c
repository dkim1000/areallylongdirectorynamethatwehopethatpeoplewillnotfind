...
thread_create(const char *name)
{
	...
	thread->t_fm = filemanager_create();

    struct processhandler *ph = processhandler_create(thread);
	thread->t_pid = pidmanager_add(pidmgr, ph);
	if (thread->t_pid < 0)
	{
		return NULL;
	}
    
	thread->t_sem_fork = (struct semaphore *) sem_create ("sem_fork", 0);
...
thread_destroy(struct thread *thread)
{
	...
	// delete the entry in the pidmanager for this process and update the pid queue
	pidmanager_remove(pidmgr, thread->t_pid);
...
thread_bootstrap(void)
{
    ...
    /* Create pidmanager */
    pidmgr = pidmanager_create();
    if (pidmgr == NULL) {
        panic("Cannot create process manager\n");
    }
...
thread_shutdown(void)
{
	...
	// Free up pidmanager space.
    pidmanager_destroy(pidmgr);
	    ...
	    struct thread **ret)
{
	...
	//newguy->t_fm = filemanager_create ();
	//kprintf ("cloning %d into %d\n", curthread->t_pid, newguy->t_pid);
	//kprintf ("size of passign fm: %d\n", filemanager_getsize((struct filemanager*)curthread->t_fm));
	// clone the parent's filemanager into the child's file manager in such a way that
	// the filedescriptors are shared.
	newguy->t_fm = filemanager_clone((struct filemanager *)curthread->t_fm);
...
thread_exit(void)
{
	...
	//filemanager_print((struct filemanager *)curthread->t_fm);
	filemanager_destroy((struct filemanager *)curthread->t_fm);
	//kprintf ("after fm\n");
	pid_t pid = curthread->t_pid;
    pidmanager_remove(pidmgr, pid);
    //kprintf ("after pm\n");

    sem_destroy(curthread->t_sem_fork);
...
/*
 * Wake up one thread who is sleeping on "sleep address" ADDR.
 */
#if OPT_A1
void
thread_wakeupone(const void *addr)
{
	...
	assert (curspl>0);
	int numSleepers = array_getnum(sleepers);
