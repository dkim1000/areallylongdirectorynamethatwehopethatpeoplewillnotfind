...
pid_t sys_waitpid(pid_t pid, int *status, int options)
{
    ...
    //kprintf("sys_waitpid: %d waiting for this pid %d\n", sys_getpid(),pid);

    // wait for process to exit
    //while (ph_child->ph_status == PH_NOT_EXITED)
    //{
        cv_wait(ph_child->ph_exitcv, ph_child->ph_exitlock);
        //*status = ph_child->ph_exitcode;
    //}

    //kprintf("sys_waitpid: %d done waiting for %d with ph_status of %d\n", sys_getpid(),pid, ph_child->ph_status);
    // set the appropriate exit code
    assert(ph_child->ph_status == PH_EXITED);

    *status = ph_child->ph_exitcode;
    //kprintf("ph_child->ph_exitcode: %d\n", ph_child->ph_exitcode);
    // release the parent lock
    lock_release(ph_child->ph_exitlock);
    //kprintf("after lock release, return pid: %d\n", pid);
	return pid;
}
