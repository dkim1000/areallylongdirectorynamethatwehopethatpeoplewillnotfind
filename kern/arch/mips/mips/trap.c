...
{
	// avoid console printing out warnings for unused variables
	(void)epc;
	(void)vaddr;
	...
	switch (code)
	{
		...
		case EX_MOD:    /* TLB Modify (write to read-only page) */
			kprintf("TLB Modify (write to read-only page)");
			break;
		case EX_TLBL:   /* TLB miss on load */
			kprintf("TLB miss on load.");
			break;
		case EX_TLBS:   /* TLB miss on store */
			kprintf("TLB miss on store.");
			break;
		case EX_ADEL:   /* Address error on load */
			kprintf("Address error on load.");
			break;
		case EX_ADES:   /* Address error on store */
			kprintf("Address error on store.");
			break;
		case EX_IBE:    /* Bus error on instruction fetch */
			kprintf("Bus error on instruction fetch.");
			break;
		case EX_DBE:    /* Bus error on data load *or* store */
			kprintf("Bus error on data load or store.");
			break;
		case EX_SYS:    /* Syscall */
			kprintf("Syscall.");
			break;
		case EX_BP:     /* Breakpoint */
			kprintf("Breakpoint.");
			break;
		case EX_RI:     /* Reserved (illegal) instruction */
			kprintf("Reserved (illegal) instruction.");
			break;
		case EX_CPU:    /* Coprocessor unusable */
			kprintf("Coprocessor unusable.");
			break;
		case EX_OVF:    /* Arithmetic overflow */
			kprintf("Arithmetic overflow.");
			break;
	}
