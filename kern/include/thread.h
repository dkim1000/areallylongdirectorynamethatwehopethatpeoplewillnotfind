...
struct thread {
    ...
    /*
     * This is the public variable for storing the process id.
     * Note: this assumes a one-to-one relationship between the process and the thread
     */
    pid_t t_pid;
    
	/*
	 * The file manager for the thread
	 */
	struct filemanager *t_fm;


	struct semaphore *t_sem_fork;
